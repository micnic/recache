...
/** @type {{ [key: number]: "directory" | "file" }} */
const types = {
	[S_IFDIR]: 'directory',
	[S_IFREG]: 'file'
};
...
class Cache extends EventEmitter {
	...
	static async createElement(cache, path, location, stats) {
		...
		/** @type {CacheElement} */
		const element = {
			content: await Cache.contentBehaviors[type](cache, path, location),
			data: {},
			location,
			path,
			stats,
		...
		const isRootElement = (location === rootSymbol);
		...
		if (type === 'directory' || (type === 'file' && isRootElement)) {
	...
	static async listenChange(cache, location) {
		...
		if (!cache._destroyed) {
			...
			if (cache._changed) {
				cache.emit(events.update, cache);
			}

			// Emit the internal release event
			cache.emit(events.release);
		}
	}
	...
	static unlinkElement(cache, location) {
		...
		if (cache.has(location)) {
			...
			cache.emit(events.unlink, element);
	 ...
	 * @param {Cache} cache
	 * @param {Mutable<CacheElement>} element
	 * @param {Stats} stats
	 * @returns {Promise<void>}
	 */
	static async updateElement(cache, element, stats) {
		...
		const getContent = Cache.contentBehaviors[type];

		// Update element content
		element.content = await getContent(cache, path, location);
		...
		if (element.type === 'directory' && isArray(content)) {
			content.forEach((child) => {

				// Check if child is still existing
				if (element.content.indexOf(child) < 0) {
		...
		// Emit change event
		cache.emit(events.change, element);
	}
	...
	static async updateLocation(cache, location) {
		...
		try {
			...
			if (stats.isDirectory() || stats.isFile()) {
				...
				if (cache._container.has(location)) {

					const element = cache._container.get(location);
	...
	static whenReleased(cache) {
		...
		return new Promise((resolve) => {
			if (!cache._ready || cache._updating) {
				cache.once(events.release, async () => {
